#!/usr/bin/bash                             # initilize users, accounts,...             groupadd -r $USER -g $GID \
&& useradd -u $UID -r -g $USER -d /home/$USER -s /bin/bash -c "$USER" $USER \
&& adduser $USER sudo \                     && mkdir -p /home/$USER \                   && chown -R $USER:$USER /home/$USER \
&& echo $USER':'$PASSWORD | chpasswd        
# Add my own public key to NX               if [ -z "$NX_PUBLICKEY" ]                   then                                                # skip                                      true
else
        HOME="/home/$USER"
